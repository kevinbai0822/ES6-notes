...
遍历器（Iterator）就是这样一种机制。它是一种接口，为各种不同的数据结构提供统一的访问机制。任何数据结构只要部署Iterator接口，就可以完成遍历操作（即依次处理该数据结构的所有成员）。Iterator 的作用有三个：
...
### 调用Iterator接口的场合
有一些场合会默认调用Iterator接口（即Symbol.iterator方法），如下：
1. 解构赋值
对数组和Set进行解构赋值的时，会默认调用Symbol.iterator方法。
```
...
// ['a', 'b', 'c', 'd']
```
扩展运算符内部就调用了Iterator接口，实际上这就提供了一种简便机制，可以将任何部署了Iterator接口的数据结构转化为数组。
...
iterator.next()  // { value: undefined, done: true }
```
上面的例子中，调用Symbol.iterator方法返回一个遍历器对象，在其上可以调用next方法实现实现对于字符串的遍历。
...
#### 类似数组的对象
类似数组的对象包括好几类。字符串、DOM NodeList对象、arguments对象。  
并不是所有类似数组的对象都有Iterator接口。一个简便的解决方法就是用Array.from()将其转化为数组。
...
```
上面的例子中，对于普通的对象，for...in循环可以遍历键名，for...of循环会报错。
